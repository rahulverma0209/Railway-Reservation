...
void welcome()																			//Welcome Function
{
	printf("\n\n");
    printf("\t\tлллллллл \n");															//219
    printf("\t\t   лл    ллл    лл ллллллл  лллллл     лл    ллл    лл \n");
	printf("\t\t   лл    лл л   лл лл    лл   лл     лл  лл  лл л   лл \n");
    printf("\t\t   лл    лл  л  лл лл    лл   лл     ллммлл  лл  л  лл \n");
    printf("\t\t   лл    лл   л лл лл    лл   лл    лл    лл лл   л лл \n");
    printf("\t\tлллллллл лл    ллл ллллллл  лллллл  лл    лл лл    ллл \n\n\n");
    
	printf("\t  лллллллл \n");
    printf("\t  лл    лл    лл     лллллл  лл       лл     лл     лл    лл    лл  \n");
	printf("\t  лл   лл   лл  лл     лл    лл       лл     лл   лл  лл  лл    лл  \n");
    printf("\t  лллллл    лллллл     лл    лл       лл  л  лл   лллллл    лллл    \n");
    printf("\t  лл   лл  лл    лл    лл    лл       лл л л лл  лл    лл    лл     \n");
    printf("\t  лл    лл лл    лл  лллллл  ллллллл  ллл   ллл  лл    лл    лл \n\n\n\n\n");
}
...
void display_reservation()																//Function to display all reservation
{
	...
	printf("\n\t\t\t\t RESERVATION\n");
	dispchar('*');
	printf("\n\nTrain no      Train name        From            To      Name    Age    Fare     Date             PNR\n\n");
	while((fscanf(p,"%d",&r.tno),fscanf(p,"%s",r.tname),fscanf(p,"%s",r.from),fscanf(p,"%s",r.dest),fscanf(p,"%s",r.pname),fscanf(p,"%d",&r.age),fscanf(p,"%d",&r.fare),fscanf(p,"%d",&r.d),fscanf(p,"%d",&r.m),fscanf(p,"%d",&r.y),fscanf(p,"%lld",&r.pnr),fscanf(p,"%lld",&r.real_pnr))==1)
	{
		printf("%d\t",r.tno);
		printf("%s\t",r.tname);
...
void train_detail()																		//Function for storing data of a train
{
	struct train t;
	FILE *p;
	p=fopen("train.txt","a");
...
void train_display()																	//Function for displaying data of a train
{
	struct train t;
	FILE *p;
	p=fopen("train.txt","r");
	...
	printf("\n\n  Train no          Train name           From           Destination      Adult fare\tChild fare\tTotal seat\n\n");
	while((fscanf(p,"%d",&t.tno),fscanf(p,"%s",&t.tname),fscanf(p,"%s",&t.from),fscanf(p,"%s",&t.dest),fscanf(p,"%d",&t.fare),fscanf(p,"%d",&t.fare_child),fscanf(p,"%d",&t.no_of_seat))==1)
	{
		printf("%10d\t",t.tno);
		printf("%20s\t",t.tname);
...
void train_modify()																		//Function for modifying data of a train
{
	struct train t;
	int key,found=0;
	FILE *p,*q;
	...
	while((fscanf(p,"%d",&t.tno),fscanf(p,"%s",&t.tname),fscanf(p,"%s",&t.from),fscanf(p,"%s",&t.dest),fscanf(p,"%d",&t.fare),fscanf(p,"%d",&t.fare_child),fscanf(p,"%d",&t.no_of_seat))==1)
	{
		if(t.tno==key)
		{
			...
			scanf("%d",&t.fare_child);
 			printf("Enter train's total no of seat : ");
	...
	if(found==0)
	printf("\n\nTrain no %d not found",key);
	
	remove("train.txt");																//deleting original file
	rename("temp.txt","train.txt");														//old_name , new_name
}

void train_delete()																		//Function for deleting data of a train
{
	struct train t;
	int found=0;
	int key;
	...
	while((fscanf(p,"%d",&t.tno),fscanf(p,"%s",&t.tname),fscanf(p,"%s",&t.from),fscanf(p,"%s",&t.dest),fscanf(p,"%d",&t.fare),fscanf(p,"%d",&t.fare_child),fscanf(p,"%d",&t.no_of_seat))==1)
	{
		if(t.tno==key)
		{
			printf("Train no %d - %s data has been deleted\n",t.tno,t.tname);
			found=1;
		}
		else
		{
